...
<h1><i class="fas fa-solid fa-wallet">&nbsp;</i>Empréstimos</h1>
@stop
@section('content')
<div class="card card-outline card-primary table-responsive">
    ...
    <div class="row table-responsive">
        <div class="col-sm-12">
            <table class="table table-hover text-nowrap dataTable no-footer" role="grid" style="width: 99%; margin-left:auto; margin-right: auto">
                ...
                <tbody>
                    ...
                    <tr class="odd">
                        ...
                        @elseif($item->status === 'Simulação' && $item->status_color === 'alert')
                        <td>
                            <span class="btn btn-warning btn-sm">{{$item->status}}</span>
                        </td>
                        @elseif($item->status === 'Simulação' && $item->status_color === 'danger')
                        <td>
                        ...
                        <td>
                            @can('edit_register')
                            <a class="btn btn-primary btn-sm" href="{{ route('cms.loan.edit', $item->id)}}"><i class="far fa-edit"></i> Atualizar</a>
                            <a class="btn btn-primary btn-sm" href="{{ route('cms.loan.historic', $item->id)}}"><i class="fa fa-file"></i> Histórico</a>
                            @endcan
                        </td>
                    </tr>
                    @endforeach
            ...
            <div class="d-flex justify-content-center">
                <b>Total de {{ $loans->total() }} registro(s)</b>
            </div>
        </div>
